...
made that a seam rather than a cut at the halfway mark (D-134's rule, D-158). What stays here is
auth, routing, session state, client data flow, and the OCR-adjacent traps that have accumulated.
...
- Symptom: the only route to the rest of a paged ledger renders as the last three words of a sentence — same colour, same size, no border or underline — and the end-to-end spec that clicks it by name passes.
...
- Symptom: a control that only means to flip one boolean is added; the write succeeds, the flag changes, every test passes — and the description, counterparty, category and note the owner had typed on that row are gone, with no error anywhere.
- Cause: `PUT /api/v1/transactions/[id]/overlay` takes the whole overlay and `update_transaction_overlay` writes it with `on conflict do update set` over **every** column. Two failure modes look nothing alike: a body naming only the changed field is **refused** by the `.strict()` schema, which is safe and loud; a body sending the rest as `null` is **accepted**, because it is indistinguishable from a legitimate clear-everything write. The dangerous one is the one that looks like success.
- Avoid: never let a caller assemble the body. Derive it from the record — `overlayWriteBody(transaction, change)` spreads the overlay in force and lets `change` narrow it — so the erasure is unrepresentable rather than something each new control has to remember, and the next field editor inherits the guarantee. Keep the request schema beside the builder rather than in the route, so a test can assert the builder's output against the contract the route really enforces. A test proving "the flag changed" passes against the erasing version: the case that matters is a **populated** record surviving the write, asserted end to end against the real function.
- Verify: 2026-08-27. With the builder returning nulls, three Vitest cases fail by name and the browser spec fails on *the description the owner typed must survive a toggle*; restored, a seeded populated overlay comes back with only `include_in_reporting` and `revision` moved (D-165).

## A total that honours a flag on one branch and not the other disagrees with itself in one keystroke

- Symptom: a row is excluded from reporting, the strip's Money in falls by its amount, the row wears an "Excluded" chip — and typing a single character in the search box brings the amount back while the chip stays.
- Cause: the ledger's strip prefers the **server's** whole-account total, which applies `coalesce(o.include_in_reporting, true)` in SQL (migration 023), and falls back to the client's `summarizeRows` the moment a text query or a confirmed-status filter narrows the population beyond what SQL was asked about. The fallback did not know about the flag. The two branches were written months apart and only one of them was updated when the flag gained a reader.
- Avoid: when a rule is added to a figure computed in two places, put it in the shared function rather than at the call site that happens to be in front of you — then neither branch can drift. Look for the *other* branch by asking what makes the preferred one invalid; here the docblock above the memo already named the two conditions, which is where the second branch was hiding in plain sight.
- Verify: 2026-08-27. Excluding a row and typing one character restored the excluded amount; with the filter inside `summarizeRows`, three unit cases pin it and all three fail by name when it is removed (D-165).

## `display: none` on an empty `aria-live` region defeats the reason it was rendered empty

- Symptom: a live region is deliberately rendered at all times so the screen reader is watching it before it has news — and the news still goes unannounced.
- Cause: the region is hidden with `display: none` while empty, and a `display: none` element is not in the accessibility tree at all. So from assistive technology's point of view it *does* appear at the same instant as its text, which is exactly the state the unconditional render exists to avoid. The code and the comment both look right; the CSS three files away is what makes them false.
- Avoid: clip it instead — `position: absolute; width: 1px; height: 1px; overflow: hidden; clip-path: inset(50%)` — which keeps it in the tree and observed while costing no layout height. Whenever a component's docblock argues that an element must always exist, check what the stylesheet does to it when it is empty.
- Verify: 2026-08-27, found by `/code-review` on the font picker's message line, which had been made unconditional for precisely this reason in the same change that hid it (D-166).

## Merging per-account pages into one timeline shows gaps unless it stops at the shallowest paged account

- Symptom: the all-accounts ledger shows a stretch of days with no rows, or far fewer than usual, and the combined balance jumps across it — while every single-account view is complete. It reads as lost transactions.
