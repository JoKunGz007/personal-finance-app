...
- Symptom: a stacked "rows become cards on a phone" mode turns a table into blocks, and one cell renders far narrower than the row it is the only child of — then its wrapped content paints straight through whatever is drawn below it. The cell's computed `display` is `block`, no rule in the phone breakpoint mentions a size, and the content that overflows reports its full height while the cell reports a smaller one.
- Cause: `display: block` changes how a box participates in layout; it does not discard a `width` or `height` resolved from elsewhere. A desktop rule sizing columns — `th:nth-child(1) { width: 115px }`, which sits outside every media query precisely *because* it sizes a column — still matches the cell after the switch, and a `colspan` heading is its row's first child, so it collects the first column's width. The height half is worse because nothing declares it: the cell's used height stayed fixed while its in-flow child grew, so the box never grew to contain its content and the overflow painted over the next element instead of being absorbed.
- Avoid: reset **both** axes in the stacked block, not only the one whose symptom you can see — `width: auto; height: auto`. Read the sibling rules first: if the `td` rule in the same block already resets `height` and `min-width`, the `th` beside it needs the same and its omission *is* the bug. Assert that the cell fills its row, which holds regardless of how long the content happens to be.
- Verify: 2026-09-04, on the deployed ledger at 390px. The heading cell measured 115×58 inside a 358px row against a 70.4px child, and 117 of 122 headings overflowed, the worst by 39.4px; with both resets it measured 358×33.8 and none overflowed. `width: auto` alone left the box at 58px, and forcing a wrap back reproduced the spill at +3.6px (D-187).

## An SVG hit target centred on an edge point runs outside the plot, and the root `<svg>` hides it

- Symptom: a chart looks correct and behaves correctly, and a layout audit reports an element escaping its container by a few pixels. Nothing on screen is wrong, no page pans, and the offending element has `fill="transparent"`.
- Cause: hover targets are usually drawn as a band centred on each data point, one band wide per point. The first and last points sit exactly on the plot's edges, so half of each end band lies outside it. This is invisible while a window holds hundreds of points and the band is a pixel or two wide, and becomes real once the band is wide — band width is `plotWidth / count`, so a short window makes it large. The outermost `<svg>` clips to its viewport by default, so the excess never paints; `getBoundingClientRect` still reports the unclipped geometry, which is what an audit sees. **The part that is clipped is the end point's own hit area**, so the visible symptom is a data point that is harder to hover than its neighbours, not a drawing error.
- Avoid: clamp each band to the plot rectangle rather than centring it unconditionally. Do not dismiss the audit's report as an SVG false positive — nothing paints wrong, but the geometry is claiming room the chart does not have, and the target it costs you is a real one. Note that centred bands of width `plotWidth / count` never quite tile either, since points are spaced `plotWidth / (count - 1)` apart; the gap is negligible at hundreds of points and is half the chart at two.
- Verify: 2026-09-04, at 390px over a twelve-day window. The last band left an 820-unit `viewBox` by 12.7 units — about 6px rendered — with a right padding of 18. Clamped at both ends, the audit passes and the end points hover across their whole visible width (D-188).

## A `td span` display rule also reaches spans nested inside the cell's text

- Symptom: text that should read on one line inside a table cell breaks at every inline span — an account label renders as label / `····` / digits on three lines, and a reference splits at each digit run.
