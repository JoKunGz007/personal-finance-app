...
- Symptom: a receipt reader whose tests all pass refuses the real full invoice with `MISSING_FIELD`, or reads it with an item missing and fails its net check.
- Cause: splitting `getTextContent` items on `hasEOL` alone. A wrapped item name ends its run with `hasEOL`, the tail follows on a lower baseline, and the same row's amounts arrive *after* the tail — one printed row becomes three lines. Right-aligned labels (`เลขที่`, the invoice's `วันที่`) share the baseline of whatever is printed at the left of their row, so a start-anchored label never matches. The reader's tests were written against text measured with a different extractor, so they never saw pdf.js's join.
- Avoid: group items by baseline, keeping pdf.js order inside a row (`lib/receipt-pdf.ts`) — never sort by x, which is what reorders Thai marks. Anchor labels on structure that survives sharing a row (the end of the line, a required letter). And run the reader end to end on the real document through the **same** extraction the app uses before calling it done.
- Verify: 2026-09-23 (D-209). The real full invoice refused on the `hasEOL` join and read complete on the baseline join; the three regression tests in `tests/receipt-pdf.test.ts` and `tests/receipt-text.test.ts` fail on the old join and anchors.
