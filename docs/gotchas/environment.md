...
- Symptom: a regex written as a Unicode range — Thai letters, zero-width spaces, non-breaking space, the private-use block — reads correctly in the tool call and is wrong on disk. A test fails because the baht sign was masked as a Thai letter, or a later `Edit` cannot find its own earlier text; `cat -A` shows raw multi-byte characters where the escape should be, and some are invisible.
- Cause: the agent's Write/Edit and a heredoc'd patch script all decoded `\u` escapes in the content before writing, so the file holds the characters themselves. The Thai range that was meant to stop before the baht sign became a literal range that did not, and the zero-width and private-use characters became invisible in any diff.
- Avoid: write a class that needs no escape — `\p{Script=Thai}` (the baht sign is `Script=Common`, so it stays out) and `\p{Co}` for private use. Where an escape is unavoidable, build the backslash from a source that cannot be re-consumed (`chr(92)` in a patch script). Then grep the written file for the invisible ranges before trusting it.
- Verify: 2026-09-23 (D-219). `lib/delivery-grab.ts`'s `lineShape` masked `฿` until the class became `\p{Script=Thai}`; `tests/delivery-grab.test.ts` pins `฿` surviving the mask. A grep for U+200B, U+00A0 and U+E000 … U+F8FF over the new files now finds nothing.

## pdf.js loads under `next start` and fails on Vercel with "DOMMatrix is not defined"

- Symptom: a route that imports `pdfjs-dist/legacy/build/pdf.mjs` passes every test and answers correctly from a local production build (`next start`), then on the deployment answers **every** request, signed out included, with Next's HTML error page and a 500, before any of its code runs.
