# Private Ledger decision log — archive, D-142 … D-152

Relocated from `DECISIONS.md` on 2026-08-26, unchanged, because that file reached **95% of its
117 KB byte budget** (D-130). Append-only still applies — a decision superseding one of these goes
in `DECISIONS.md` and references it here.

**This is the seventh boundary, and it is the first that does not start where the sixth stopped.**
Every previous range was contiguous from the last one's end. This one begins at **D-142** and
leaves **D-141 in `DECISIONS.md`**, alone above D-153.

The reason is the rule itself. D-133 set it — *a boundary excludes every open question* — and D-140
sharpened it into a test: *a question is closed when the code has stopped asking it*. D-141 is still
asking. Whether the mailbox source is deleted after import, and so whether it becomes a permanent
archive under a password derived from a citizen ID and therefore non-rotatable, is **deferred and
unanswered by the owner**. The sixth boundary stopped at D-140 for exactly that reason and said so.

**What changed is not the rule but the shape.** Waiting for D-141 to close meant the budget breaching
first: eleven entries had accumulated behind one deferred question, and the next entry would have
failed `check:docs --strict` mid-task. Stepping over the open question rather than stopping short of
it keeps the rule intact — nothing unresolved has been filed away — at the cost of a gap. The owner
chose this over raising the budget, which is the precedent D-134 set once for `GOTCHAS.md` and D-149
then paid off by splitting instead.

**The gap is safe because `scripts/check-docs.mjs` pools the maintained file with every archive** and
checks ids for duplicates and omissions across the whole pool. It never required the maintained file
to be contiguous; six boundaries in a row simply happened to leave it that way.

What this range covers: bulk statement import and the mailbox arc (D-141 excepted), the slip-form
review fixes, the tertiary button rank, automatic account binding, the fifth and sixth archive
boundaries, the client wire seam, the `GOTCHAS.md` split, and the two architecture candidates that
closed last week's defect classes.
...
- Date: 2026-08-24
- Status: **Done.** **D-130 … D-133** relocated unchanged to [`docs/decisions/ARCHIVE-D-130-D-133.md`](ARCHIVE-D-130-D-133.md). No entry was rewritten, no index bullet was lost, and every cross-reference still resolves — `check:docs --strict` reads the archives, so a reference to an archived id is not a dangling one.
