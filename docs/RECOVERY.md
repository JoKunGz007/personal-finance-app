...
The app itself can now take a real backup and restore one — the **`/recovery`** route, available to a signed-in owner whether or not a statement is open. It was § Recovery / 04 on the single page until routing split the app into four routes on 2026-07-31.
...
Keep the file and its password apart, and note what the pair means: either alone is useless, and losing both makes the ledger unrecoverable. The `.pldemo` preview offered beside a parsed synthetic statement on `/import` is a different artifact entirely and is not restorable.
