...
- Avoid: when adding a table, do not reason from the migration text. Either repeat migration 002's `revoke all on ... from anon, authenticated` for the specific new tables before granting `select` back, or accept the gap knowingly. Do not write `revoke insert, update, delete` and believe it did something.
