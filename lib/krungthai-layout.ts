...
import {
  LINE_TOLERANCE, groupIntoLines, maskShape,
...
// The text-layer types and the four value-free diagnostics live in
// `lib/masked-diagnostics.ts`, which has no imports of its own so that
// `scripts/mask-statement.mjs` can run them under plain Node (D-035). They are
// re-exported here because the worker, the fixtures, and the tests all reach the reader
// and its diagnostics through this one module.
