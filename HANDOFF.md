...
- **The slip-form review fixes are BUILT AND UNCOMMITTED, 2026-08-23** (D-142): `lib/slip-batch.ts`, `app/slip-batch.tsx`, `tests/slip-batch.test.ts`, `tests/privacy.test.ts`. Eight defects `/code-review` found in `7be667e`'s two files. No SQL, no route. **Gate re-run in full and green**: Vitest **643 passed / 7 skipped across 32 files**, Playwright owner **31/31** and isolated **18/18**, production build clean at eighteen routes, tsc and ESLint clean. pgTAP deliberately not re-run — 266 across 8 from earlier the same day, and nothing here moves SQL.
...
- **Phone width is measured for BOTH batch worklists as of 2026-08-23**, by the throwaway `.runtime/worklist-phone-audit.spec.ts`. It signs in at 390px, feeds synthetic files through each form, **asserts rows are on the page**, and only then measures — the D-138 lesson, since an audit that walks routes cannot see a list that does not exist until files are read. **The statement worklist is clean.** **The slip worklist does not overflow but has two tap targets under 44px**: the unclassed `Discard` button (`app/slip-batch.tsx:464`) at 24px tall, and `.batch-fix input` at `height: 42px` (`app/globals.css:242`). Both predate today, **neither is fixed, and both are the owner's call** — PLAN task 28 never agreed a standard.
