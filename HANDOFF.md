...
- **Direct access to `private-statements/`: granted 2026-07-25, and NOT YET USED.** The owner offered three times; the first two were declined. The grant came with a correction they accepted: they believed agent reads keep data on-device, which is false — the app's on-device guarantee (D-003) belongs to the app, not to an agent, whose reads are transmitted and retained in a session transcript. **Two gates must be satisfied before any use:** a `DECISIONS.md` entry recording the grant and its boundary, and an explicit amendment to `docs/FIXTURE_POLICY.md`. Neither is written yet, so the directory remains off limits. The agreed boundary is **invoke, don't read**: a local masking harness opens the PDF and emits only masked structure, so raw content stays in a process on the owner's machine. Statement passwords are the owner's date of birth and citizen ID — identity-grade and non-rotatable. Never accept one in chat, in a repo file, or as a CLI argument; the harness reads stdin only and needs it once per file.
...
- Run `git status --short`. Four config files (`.gitignore`, `eslint.config.mjs`, `playwright.config.ts`, `pnpm-workspace.yaml`) are **intentionally uncommitted** — preserve them. Everything else is committed **and pushed**: `main` is level with `origin/main` at `cfa24d8`, which carries the D-028 frame-boundary fix, the D-029…D-032 parser corrections from the real-statement reads, and the D-033/D-034 cross-check and currency guard. Treat commit and push authorization as spent; ask again.
