...
[DECISIONS.md](DECISIONS.md) (append-only; indexed at the top, carrying **D-141, D-158 and D-223 … D-230**
...
questions, the newest entries (D-223 … D-226, D-228 … D-230) and the boundary's own record of itself (D-227)** — the mailbox archive (D-141), deferred by
...
- **Real-data read, commit, push, `db push` and hosted web use (the pane and Claude in Chrome): GRANTED and SPENT, 2026-09-25 (the D-230 session)**, in one line from the owner. Spent on hosted reads of counts and function grants only, `db push` of 041 after 466 / 466, `4198dcb` and its docs commit, their pushes, and a live check plus one Sync in the signed-in pane (no new mail). **None of this survives into a new session.**
