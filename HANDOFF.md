...
Task 12 is closed too (D-043, below), so **every local task in `PLAN.md` is now done**. What is left is gated rather than unbuilt: **portable recovery into an empty separately bound project** (§ Later authorization gates item 3), which is the binding gate on importing anything real; and **task 13, receipts**, behind a CSP spike for whether tesseract.js runs under this policy at all (D-037). Recovery is the one worth doing next — it unblocks the only thing this app exists to do.
...
- **Commit and push: granted 2026-07-25 and used repeatedly**, most recently on 2026-07-27 to push the SCB and KBANK work through `bce402b`. Treat as spent at the start of a new session; ask again.
...
- `main` is level with `origin/main` at `bce402b`, pushed 2026-07-27: the SCB and KBANK readers and their corrected contracts, migration 009, the cross-check reporting of D-042, and the docs recording the first real read of each layout. Four commits on top of `07c7ab7`. Treat commit and push authorization as spent; ask again.
