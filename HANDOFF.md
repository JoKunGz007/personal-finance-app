...
- **The archive rate is the thing to watch, and it is faster than "roughly a fortnight".**
  ...
  which is not true here, so this one is archived instead. **It is at 89% (105 KB/117 KB) as of 2026-08-26**, after the sixth boundary took it to 72% and four entries put it back. **The seventh boundary is still bounded at D-141**, because whether the mailbox source is deleted after import is deferred and unanswered — so the next reader either gets that decision or raises the budget.
...
- **The commit-and-push grant is spent on `a123929`, pushed 2026-08-26.** Two pushes were granted and used this day — `2da46fe` (D-148, D-149, D-150) and `a123929` (D-151). **Eight production deployments now stand unverified in the dashboard.** The next push needs a new ask.
- **D-152 is DONE, GATED AND UNCOMMITTED.** The card capture restructure: new `lib/notification-card-form.ts` and `tests/notification-card-form.test.ts`, plus `app/notification-card-capture.tsx`, `tests/privacy.test.ts`, `docs/gotchas/app.md`, `DECISIONS.md` and `PLAN.md`. **No SQL, no route, no contract and no CSP change**, so `/security-review` is not owed — the reader route, the Vision call and the stored key were all left alone. Alongside it the tree holds this file's own edits and the **two deliberately local-only config files**; `git status --short` is what tells them apart.
