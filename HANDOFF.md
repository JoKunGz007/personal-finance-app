...
[7-Eleven](docs/RECEIPT_CONTRACT.md) — format knowledge for `PLAN.md` task 56, whose PDF capture path is
...
- **Sending the real receipt screenshots to Google Cloud Vision: GRANTED and SPENT, 2026-09-23 (the D-210 session)**, after the owner asked for quota and cost to be checked first (done read-only in his console in Claude in Chrome). Used once for the 21 screenshots plus four in browser checks. **Not granted: `db push` for 029** — ask, with a fresh backup. **None of this survives into a new session.**
