...
- **As of the D-201 session, read `git log` — this line cannot record the commit that carries it.**
  D-201's code (`0e486b0`, the all-accounts ledger stops at the paging floor) is the newest change
  to what the app serves, **confirmed live**. Beneath it, D-200's code (`baae8e5`, only a digit run takes the figures font, never a word beside it) is the
