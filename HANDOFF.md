...
- **Commit, push, live verification: GRANTED and SPENT, 2026-09-16 (the D-205 session).** Asked for explicitly ("yes, review, commit, push and verify live") after the fixes were presented. Spent on `3cd055a` and `b422c6f` and the docs commit after them; the live read in the browser pane measured structure only and recorded no figures. **None of this survives into a new session — ask again.**
...
- **As of the D-201 session, read `git log` — this line cannot record the commit that carries it.**
  D-205's code (`3cd055a`, `b422c6f`, the third /ux-review pass) is the newest change to what the app serves,
  **confirmed live**. Beneath it, D-204's (`d8ca844`, the second pass), also confirmed live. Beneath it, D-203's (`8294e22`, the first pass), also confirmed live. Beneath it, D-202's (`bda7ba0`, digit-only figures face webwide; long help
