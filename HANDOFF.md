...
- **Importing a real statement: never standing, ask every time.** One KBANK statement was imported on 2026-07-28 (D-047). That was a decision taken once, not a permission. Note what the first one cost in vigilance: it silently turned `pnpm test` into a destructive command, and the fix took a whole second project (D-048). Assume the next import creates a hazard nobody has thought of yet, and go looking for it.
...
- **Commit and push: granted 2026-07-28 and spent.** Used to push through `94a0144`, leaving `main` level with `origin/main`. Treat as spent at the start of a new session and ask again.
...
- Run `git status --short`. Two kinds of uncommitted file are in the tree and they are not the same thing. **Three config files** — `eslint.config.mjs`, `playwright.config.ts`, `pnpm-workspace.yaml` — are **deliberately local-only**; preserve them, do not commit them. Everything else uncommitted is ordinary work awaiting authorization to commit: as of 2026-07-28 that is the task 17 transactions view and the dev-session per-project secret store, across `app/`, `lib/`, `tests/` and the continuity docs.
