...
**D-177 … D-186 without D-179 … D-181, D-183 and D-184** to
[`docs/decisions/ARCHIVE-D-177-D-186.md`](docs/decisions/ARCHIVE-D-177-D-186.md) on 2026-09-04. The
index below covers all fourteen files, so a reader can find any entry without opening any body.
...
**What this file now holds is two open questions, four entries behind a reading that only half
happened, and the entry reporting it.** The mailbox archive (D-141) and the unbounded candidate scan
(D-158) remain open and are the whole of what is unanswered. **The phone reading arrived on
2026-09-04, and it neither freed everything it was expected to nor came back clean**: it covered
`/ledger`, found a real defect there, and that defect is D-187. **D-179 and D-183 are `/calendar` and
were not in the capture at all; D-180 and D-181's four schemes were seen only in the one the owner
had on; D-184 answers for D-183 as well as for D-182 and cannot leave while half its subject is
fenced.** Those four are still held — but the fence is now **named rather than blanket**: a
`/calendar` reading and a pass through the four schemes, both at 390px on a device.
...
- **D-191** — Confirming a statement import currently doubles every row on `main`, reproduced on a clean worktree with no relation to D-190, and left open for the next session

## D-191 — Confirming a statement import currently doubles every row on `main`, reproduced on a clean worktree with no relation to D-190, and left open for the next session

- Date: 2026-09-12
- Status: **Open. Discovered, isolated, not yet root-caused or fixed.**
- Context: while gating D-190 for commit, `tests/e2e/owner-session.spec.ts`'s `reads a confirmed import back, and switches between merged and per-account` failed — importing a 4-row synthetic statement through the real "Confirm import" button landed **8** rows, exactly double, with no "could not be confirmed" refusal shown. The confirm believes it succeeded once; the ledger holds two copies.
- Isolation: reproduced against `main` at `5d8ba83` (the commit immediately before D-190, via a throwaway `git worktree`, no categories code present) with a **freshly `supabase db reset`** local project immediately before the single test ran alone. Same result: 8 rows for a 4-row statement. This rules out both D-190's code and cross-suite state pollution from running several Playwright/Vitest/pgTAP suites concurrently earlier in the session (a real, separate collision this session also hit and resolved by rerunning everything sequentially against a fresh reset — see `HANDOFF.md`). The doubling survives a clean database and clean code, so it is neither.
- Not yet done: reading whether the client fires `POST /api/v1/imports/confirm` twice (a double-submit) or whether `confirm_import` itself inserts twice server-side for one call; checking whether this is new or has been silently broken since some earlier commit, given the owner Playwright suite has not run successfully since D-187/D-188 on 2026-09-04 — every session between then and D-189 either skipped it (no Docker) or, this session, hit it and moved on without diagnosing it first.
- Evidence: two isolated single-test runs (`-g "reads a confirmed import back"`), both against a database reset immediately beforehand, both landing 8 rows for a 4-row import — one on the working tree with D-190 present, one on a `git worktree` at `5d8ba83` with it absent. Neither run's database or code differed in any way relevant to import confirmation.
