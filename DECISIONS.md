...
[`docs/decisions/ARCHIVE-D-134-D-140.md`](docs/decisions/ARCHIVE-D-134-D-140.md) — a superseding entry has to stay findable from the maintained file for as long as anyone might read the entry it overturns. And it carries **an owed reading of its own**: no dark scheme has been seen against real rows, status chips, the verified rail or the calendar, because the local project's ledger is empty and the deployed app is where those surfaces exist. That fence expires the moment the deployment is looked at in a dark scheme — the same shape as D-171's, and the tenth boundary's lesson was that such a fence outlives its own expiry unless the header is re-read.
