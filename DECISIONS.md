...
What this file now holds is three open questions and the live frontier — the mailbox archive, an unbounded candidate scan, and the account filter half of the statistics surface, **reviewed, built and committed as of 2026-08-31 (D-177) but still open pending deployment** — then the two changes that shipped on 2026-08-29 and that nobody has looked at where they render: the default face and the statistics window picker.
...
- **D-177** — Task 46's account filter gets its control, reviewed and reused between `/ledger` and `/statistics`, closing D-161's second half locally — committed, not yet deployed
