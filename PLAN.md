...
| Playwright, owner config | Passed, 6/6 — the binding chooser, a refused non-matching binding, an out-of-charset statement refused at assembly (D-036), an SCB statement carried through the chooser into `confirm_import`, an SCB statement refused when bound to a Krungthai account sharing its last four (D-041), and an account created from the bind stage for a statement no existing account could accept (D-045) |
...
Tasks 1–12 and 15 are complete and verified for statements. Remaining: task 13 (receipts, behind an OCR spike) and task 14 (a recovery surface a person can drive).
