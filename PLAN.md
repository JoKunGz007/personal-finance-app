...
3. Bind an extracted statement to a ledger account and assemble the `ImportPayload`. The grid and the frame both extract now (D-015/D-016: `lib/krungthai-layout.ts`, 27 tests), including account type, last four, period, opening/closing balances, THB, and a closing-balance cross-check. What remains is the account mapping — which `accounts` row an extracted last-four belongs to is a user decision the parser must not infer — plus computing fingerprints and the confirmation digest client-side and posting to `/api/v1/imports/confirm`.
