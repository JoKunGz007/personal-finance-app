...
describe("restore manifest", () => {
  ...
  it("describes a written backup from the snapshot rather than from the newest table list", () => {
    ...
    const v10 = describeBackupSnapshot({ schemaVersion: 10, tableCounts: countsFor(BACKUP_TABLE_KINDS_V10) });
    expect(v10).toContain(`${BACKUP_TABLE_KINDS_V10.length} tables`);
    ...
    const v12 = describeBackupSnapshot({ schemaVersion: 12, tableCounts: countsFor(BACKUP_TABLE_KINDS_V12) });
    expect(v12).toContain(`${BACKUP_TABLE_KINDS_V12.length} tables`);
