-- Migration 028 — the backup half of migration 027 (PLAN task 56, D-decision recorded
-- 2026-09-22/23). Migration 027's own header said this in advance: the three receipt tables
-- are not yet carried by `export_backup_snapshot` / `restore_backup`, and widening the backup
-- contract is its own decision surface, done here as a deliberate second pass.
--
-- v7 -> v8. Three new owner tables (`receipts`, `receipt_items`, `receipt_discounts`), no new
-- column on anything that already existed, which is D-097's rule kept once more. Appended,
-- never slotted in: the manifest binds a chunk to its index, so reordering would invalidate
-- every existing descriptor in a way no digest could distinguish from tampering. Indices 0..23
-- keep meaning exactly what they meant in v7 — `lib/backup-contract.ts`'s
-- `BACKUP_TABLE_KINDS_V7` names them. Parent before children within the new three: the restore
-- inserts in list order and both children carry a `receipt_id` foreign key into `receipts`, so
-- `receipts, receipt_items, receipt_discounts` is a correctness requirement rather than a style
-- choice, exactly as `slips` before `slip_match_overlays` was.
--
-- v2 through v7 all stay restorable forever, for the reason that has not weakened once: a
-- version that stops being restorable strands whatever the owner took under it.
--
-- Money crosses exactly as every other kind's does: `bigint` in the table, canonical int64 text
-- in the exported JSON and in a restored row, checked with `private.is_canonical_int64_text`
-- before any cast. `receipts` carries five nullable money columns
-- (`subtotal_minor`, `vat_pre_minor`, `vat_minor`, `vat_total_minor`, plus `net_minor` which is
-- not null) and `receipt_items.unit_price_minor` is nullable too — each is checked only when
-- present, the same shape `cash_entry_overlays` and the notification-card correction overlay
-- use for their own nullable money columns, applied here per column rather than assuming a
-- single flag covers all of them.
--
-- Owner identity is rebound on restore exactly as every other table's is: the caller's own id
-- is substituted in, never the payload's. None of these three tables embeds an owner id inside
-- jsonb (no `snapshot` column here, unlike a revisions table), so there is no second place to
-- rebind — restated because migration 017's header made the same point about its own tables and
-- a reader checking this migration should not have to wonder whether a jsonb rebind was missed.

begin;

alter table public.restore_runs
  drop constraint if exists restore_runs_schema_version_check,
  add constraint restore_runs_schema_version_check check (schema_version in (1,2,3,4,5,6,7,8));
...
begin
  ...
  return jsonb_build_object('schemaVersion',8,'exportedAt',to_char(v_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),
...
-- The kind list is built up by version, so a ninth version adds one line and strands nothing.
...
begin
 ...
 if v_schema_text not in ('2','3','4','5','6','7','8') or v_digest!~'^[a-f0-9]{64}$' then raise exception 'invalid restore contract'; end if;
 ...
 if v_run.status<>'staged' then raise exception 'restore is not staged'; end if;
  if exists(select 1 from public.accounts where owner_id=v_owner)
    ...
    -- The three new ones, for the reason every table above is listed: the question is whether
    -- this ledger already holds anything, not whether the payload has an opinion.
