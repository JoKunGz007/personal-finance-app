-- Bind the import payload digest to the persisted facts.
--
-- Review blocker #1: confirm_import trusted the caller-supplied payload digest as
-- the sole "same payload" discriminator, so materially different rows could be
-- blessed under an unchanged claimed digest (the artifact-reuse path returned the
-- original batch and silently discarded the divergent rows). The digest is a
-- security/idempotency/audit anchor and must be a function of the actual payload.
--
-- The server now recomputes the canonical digest over the statement frame plus the
-- exact rows it receives (private.canonical_jsonb mirrors lib/canonical.ts), rejects
-- any mismatch with the caller-supplied value (verify, never trust), and uses the
-- recomputed digest as the stored/compared anchor. The client computes the identical
-- object in app/api/v1/imports/confirm/route.ts.
...
begin
  ...
  -- Recompute the canonical digest over the frame + the exact rows and bind the
  -- caller's claim to it. Any divergence in rows or frame changes v_digest, so a
  -- reused claimed digest over different content fails closed here.
