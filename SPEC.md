...
Status as of 2026-07-25: gates 1, 2, 4, and 5 pass against synthetic data — re-verified after the account-binding UI and route-wrapper coverage landed (D-021, D-022) — and gate 3 is met for the transaction grid and statement frame by the invented fixtures in `tests/fixtures/krungthai-layout-v1.ts` (DECISIONS D-015, D-016). Gate 3 remains only partly discharged in substance: the fixture geometry was invented rather than measured, so it validates the parser contract without establishing that a real Krungthai layout matches it. The authorized smoke tests are the only check of that. Six attempts on 2026-07-25 each found a defect rather than an answer (D-023 … D-026, D-028), and gate 3 is now **partly discharged against reality**: a real statement opens, decodes, matches its bank signature, matches all seven column headings, states its currency where the reader looks, and yields its account type and statement period. The seventh defect those attempts exposed is fixed (D-028) and no defect is open, but the statement has not been re-read since; row-level geometry, the date format inside the `Date/Time` cell, and the account number remain unverified against reality. See `PLAN.md` task 7.
