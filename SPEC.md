...
Private Ledger is a local-first, single-owner application that converts a supported bank statement — Krungthai, SCB, or KBANK — into a reconciled, reviewable ledger without exposing the PDF, its password, or private financial data. Bank slips are images, captured through the app as provisional entries the statement later confirms (D-050, `PLAN.md` tasks 20–22); the OCR-first framing of D-037 is superseded, and cash receipts are entered by hand.
