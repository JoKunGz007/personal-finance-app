...
Gate 6 passes locally as of 2026-07-27 (D-044): a ledger exported from the primary project restores into `private-ledger-recovery`, a second project bound to a different owner, with every identity rebound — including one embedded inside `overlay_revisions.snapshot` that no column-level check reaches. That discharges `PLAN.md` § Later authorization gates item 3 locally. It does **not** relax "committed data remains invented only": importing a real statement remains the owner's explicit decision, no hosted recovery has been rehearsed, and the app still has no restore surface a person can drive (`PLAN.md` task 14).
